          ...
          <CardContent className="p-3 pt-0 sm:p-6 sm:pt-0">
            ...
            {chosenIdx === null && !completedIds.has(scenario.id) && (
              <div className="space-y-2">
                {choices.map((c, i) => (
                  <Button key={i} variant="outline" className="w-full text-right justify-start h-auto py-2.5 px-3 text-xs sm:text-sm leading-snug break-words whitespace-normal hover:bg-accent hover:text-accent-foreground" onClick={() => setChosenIdx(i)}>
                    {c}
                  </Button>
                ))}
              </div>
